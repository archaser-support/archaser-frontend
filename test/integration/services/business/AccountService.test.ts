    ...
    afterEach(async () => {
        ...
        if (testAccountId) {
            try {
                ...
                console.log("Customer cleanup error (ignored):", error);
            }
            testAccountId = 0;
        }
        ...
        try {
            ...
            console.log("Dummy customer cleanup error (ignored):", error);
        }
    ...
    describe("createCustomer", () => {
        ...
        it("should throw error for existing subdomain", async () => {
            ...
            const customerData1 = {
                ...
                email_from_name: "Test Customer 1",
                category_after_automated: "Automated" as const,
                wait_days_after_automated: 1,
            };

            const customer1 =
                await AccountService.createCustomer(customerData1);
            testAccountId = customer1.id;

            // Try to create another customer with the same subdomain
            const customerData2 = {
                name: "Test Customer 2",
                company_number: "TEST-002",
                status: "Active" as const,
                promise_to_pay: 14,
                client_type: "Company" as const,
                default_first_activity_delay_days: 7,
                ...
                email_from_name: "Test Customer 2",
                category_after_automated: "Automated" as const,
                wait_days_after_automated: 1,
            };

            await expect(
                AccountService.createCustomer(customerData2)
            ).rejects.toThrow("Subdomain");
        });

        it("should handle customer creation with minimal required fields", async () => {
            const customerData = {
        ...
        it("should set correct days_after_start for first automated step", async () => {
            ...
            // Verify the first automated step has the correct days_after_start
            const firstSequence = await prisma.activitiesSequence.findFirst({
                where: {
                    account_id: customer.id,
                    step: 1,
                    category: "Automated",
    ...
    describe("getCustomerBySubdomain", () => {
        it("should find customer by subdomain", async () => {
            const customerData = {
                ...
                company_number: "TEST-SUBDOMAIN-001",
                status: "Active" as const,
                promise_to_pay: 14,
                client_type: "Company" as const,
                default_first_activity_delay_days: 7,
            ...
            const foundCustomer = await AccountService.getCustomerBySubdomain(
                "test-subdomain-lookup"
            );

            expect(foundCustomer).toBeDefined();
            expect(foundAccount?.id).toBe(customer.id);
            expect(foundAccount?.name).toBe(customer.name);
            expect(foundAccount?.sub_domain).toBe(customer.sub_domain);
        });
        ...
        it("should only return active customers", async () => {
            const customerData = {
                ...
                company_number: "TEST-INACTIVE-001",
                ...
                currency: "USD",
                email_from: "inactive@example.com",
                email_from_name: "Inactive Test Customer",
                category_after_automated: "Automated" as const,
                wait_days_after_automated: 1,
            };
            ...
            const foundCustomer =
                await AccountService.getCustomerBySubdomain("inactive-test");
            expect(foundCustomer).toBeNull();
        });
    });
});
