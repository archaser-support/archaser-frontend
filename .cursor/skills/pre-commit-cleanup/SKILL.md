...
2. Cluster paths by **concern**, not only by folder or file extension. Prefer signals in this order:
   ...
   - Same bug fix or behavior change
   - Shared types / contracts that only exist to support one feature (keep with that feature)
   - Pure tooling, plans, skills, or docs (separate from product code when independent)
3. Keep tightly coupled files together (e.g. portal component + its CSS module + gateway route; Apex class + LWC that calls it).
...
- **Topic** — short label (e.g. `portfolio health intro overlay`)
