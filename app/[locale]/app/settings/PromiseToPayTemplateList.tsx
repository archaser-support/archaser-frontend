...
const PromiseToPayTemplateList: React.FC<{ accountId?: number }> = ({
    accountId,
}) => {
    ...
    // Export handler for promise to pay templates - moved after rows definition
    ...
    const handleStatusToggle = async (id: number, currentStatus: boolean) => {
        try {
            if (currentStatus) {
                ...
                if (isInUse) {
                    showToast(
                        t("messages.activity_templates_template_in_use_error", {
            ...
            if (!response.ok) {
                throw new Error(
                    t("messages.activity_templates_error_error_updating_status")
                );
            }
            ...
            showToast(
                error.message ||
                    t("activityTemplates.error.error_updating_status"),
                "error"
            );
        }
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (!checkResponse.ok) {
                throw new Error(
                    t("activityTemplates.error.error_checking_template_usage")
                );
            }
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    errorData.error ||
                        t(
                            "messages.activity_templates_error_template_save_error"
                        )
                );
            }
            ...
            showToast(
                error.message ||
                    t(
                        "activityTemplates.error.error_deleting_activity_template"
                    ),
                "error"
            );
        }
    ...
    const promiseToPayTemplates = useMemo(() => {
        // Filter for Promise_to_pay templates only
        return rows.filter((template) => template.category === "Promise_to_pay");
    }, [rows]);
        ...
        () => [
            {
                ...
                renderCell: (params) => {
                    ...
                    // Always use the promise to pay tab URL as backUrl
                    const backUrl = `/${locale}/app/settings?tab=templates&templateType=promiseToPay`;
                    const templateUrl = `/${locale}/app/settings/promise-to-pay-templates/${templateId}?backUrl=${encodeURIComponent(backUrl)}`;
                    return (
                        <MuiLink
                            component={Link}
                            href={templateUrl}
    ...
    // Add Promise to Pay Template Button Component
    const AddPromiseToPayTemplateButton = React.memo(() => {
        const { t, i18n } = useTranslation([
            "settings",
            "common",
            "activity_templates",
            ...
            >
                <IconButton
                    ...
                    onClick={() => {
                        ...
                        const backUrl = `/${locale}/app/settings?tab=templates&templateType=promiseToPay`;
                        router.push(
                            `/${locale}/app/settings/promise-to-pay-templates/create?backUrl=${encodeURIComponent(backUrl)}`
                        );
                    }}
    ...
    AddPromiseToPayTemplateButton.displayName = "AddPromiseToPayTemplateButton";

    const customButtons = <AddPromiseToPayTemplateButton />;
    ...
    return (
        <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
            ...
            >
                {t("fields.promise_to_pay_description", {
                    ns: "promise_to_pay",
                })}
            </Typography>
            <Box
            ...
            >
                <EndlessScrollDataGrid
                    ...
                    noRowsMessage={t("fields.promise_to_pay_templates_empty", {
                        ns: "promise_to_pay",
                    })}
                    noRowsDescription={t(
                        "fields.promise_to_pay_templates_description",
                        { ns: "promise_to_pay" }
                    )}
                    onExport={handleExport}
                    exportContextInfo={{
                        pageName: "promise_to_pay_templates",
                        customPrefix: "promise_to_pay_templates_export",
                    }}
                />
            </Box>
...
export default PromiseToPayTemplateList;
